...
def customMove(gameState):
    ...
    if len(hit)==0:
        ...
        for l in afloat:
            for i in range(n):
                for j in range(n):
                    h1 = checkShip(i,j,gameState["OppBoard"],l,"V")
                    h2 = checkShip(i,j,gameState["OppBoard"],l,"H")
    ...
    for val in ordered:
        ...
        for ii in same:
            i = ii[0]
            j = ii[1]
            pos = getSpecific(i,j,gameState["OppBoard"],p[i][j][1][0],p[i][j][1][1])
            if  pos != [-1,-1]:
                print(i,j,p[i][j])
...
def choosePosRandomValidTarget(board,afloat):
    ...
    for val in ordered:
        ...
        random.shuffle(same)
        print(same[0])
        return translateMove(same[0][0],same[0][1])
...
def checkShip(i, j, board, length, orientation):
    ...
    if orientation == "V":
        if i + length - 1 >= len(board):
            return -1
        for l in range(length):
            if board[i+l][j]=="M" or board[i+l][j]=="LM" or board[i+l][j] =="L" or len(board[i+l][j])==2:
                return 0
            if board[i+l][j] == "H":
                hits_count+=1
    else:
        if j + length - 1 >= len(board[0]):
            return -1
        for l in range(length):
            if board[i][j+l]=="M" or board[i][j+l]=="LM" or board[i][j+l] =="L" or len(board[i][j+l])==2:
                return 0
            if board[i][j + l] == "H":
                hits_count+=1
    return hits_count
